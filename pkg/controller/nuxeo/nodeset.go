...
func reconcileNodeSet(r *ReconcileNuxeo, nodeSet v1alpha1.NodeSet, instance *v1alpha1.Nuxeo, revProxy v1alpha1.RevProxySpec, reqLogger logr.Logger) (reconcile.Result, error) {
	...
	expected := r.defaultDeployment(instance, depName, nodeSet.Replicas, nodeSet.Interactive, revProxy)
	//d1 := util.SpewObject(nodeSet.PodTemplate.Spec)
	//d2 := util.SpewObject(corev1.PodSpec{})
	//_ = d1; _ = d2
	if util.HashObject(nodeSet.PodTemplate.Spec) != util.HashObject(corev1.PodSpec{}) {
		...
		// the hard-coded default pod template spec generated by the 'defaultDeployment' function above
		nodeSet.PodTemplate.Spec.DeepCopyInto(&expected.Spec.Template.Spec)
	}
	//println(util.SpewObject(expected))
	...
	if !equality.Semantic.DeepDerivative(expected.Spec, actual.Spec) {
		...
		r.client.Update(context.TODO(), actual) // TODO HANDLE ERROR
	}
...
func (r *ReconcileNuxeo) defaultDeployment(nux *v1alpha1.Nuxeo, depName string, replicas int, interactive bool, revProxy v1alpha1.RevProxySpec) *v1.Deployment {
	...
	dep := &v1.Deployment{
		...
		Spec: v1.DeploymentSpec{
			Selector: &metav1.LabelSelector{
				MatchLabels: labelsForNuxeo(nux, interactive),
			},
			Replicas: &replicas32,
			Template: corev1.PodTemplateSpec{
				ObjectMeta: metav1.ObjectMeta{
					Labels: labelsForNuxeo(nux, interactive),
				},
				Spec: corev1.PodSpec{
