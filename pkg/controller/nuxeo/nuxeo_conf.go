...
// cluster state is not modified. The use case is: the configurer creates a CR with inlined nuxeo.conf. The operator
// creates a ConfigMap. The configurer then edits the CR to remove the inlined content. Then the operator removes
// the ConfigMap.
//
// Note that if the Nuxeo CR contains a nuxeo.conf configuration entry with 'ValueFrom' defined, and that name matches
// the internally defined name generated by this function, and it is owned by the Nuxeo CR then the ConfigMap
// will be removed by this function. So - configurers should not name their nuxeo.conf ConfigMap the way this
// function does.
